			...
			<script type="text/javascript">  
			   ...
			   {  
					var data = google.visualization.arrayToDataTable([  
							  ['Status', 'Number'],  
							  <?php  
							  $query = "SELECT vent_sympt_det, count(*) as d_numb FROM tbl_huf WHERE vent_dt_iuc BETWEEN '$m-01' AND '$m-31' GROUP BY vent_sympt_det";  
							  $result = mysqli_query($connect, $query); 
								  
							  while($row = mysqli_fetch_array($result))  
							  {  
								   echo "['".$row["vent_sympt_det"]."', ".$row["d_numb"]."],";  
							  }
							  ?>  
						 ]);  
					var options = {  
				...
				<table id="datatable">
					...
					<tbody>
						<?php 
							//index.php
							$mon = date('Y-m');
							$query = "SELECT DISTINCT date(vent_dt_iuc) as dat FROM tbl_huf WHERE date(vent_dt_iuc) BETWEEN '$mon-01' AND '$mon-31'";
							$result = mysqli_query($connect, $query);
							while($row = mysqli_fetch_array($result))
							{
								$query1 = "SELECT huf_id, count(*) as vid FROM tbl_huf WHERE date(vent_dt_iuc) = '".$row["dat"]."'";
								$result1 = mysqli_query($connect, $query1);
								$row1 = mysqli_fetch_array($result1);
								
								$query2 = "SELECT vent_spc, COUNT(*) as spc FROM tbl_huf WHERE vent_spc = 'Yes' AND date(vent_dt_iuc) = '".$row["dat"]."'";
