<?php
   ...
   $id=$_GET['id'];
	$qry = "SELECT * FROM `tbl_envmntl_sfty_chklst` 
	        WHERE `envmntl_sfty_chklst_id`='$id'";
		$res = mysqli_query($connect, $qry);
		$mem=mysqli_fetch_array($res);
		$count = $res->num_rows;
	

        $row[0]= 'object_can_harm';
		$row[1]= 'staircase_and_bulding_safe';
		$row[2]= 'terraces_shall_locked_secured';
		$row[3]= 'wanter_tanks_secured_looked';
		$row[4]= 'glasses_securd_head';
		$row[5]= 'bulding_secured_design';
		$row[6]= 'hospital_building_permission';
		$row[7]= 'lift_shall_licensed_mantained';
...
<table cellspacing="0" border="0" class="table-show">
	...
	<tr>
		...
		<td style="border-top: 1px solid #000000; border-bottom: 1px solid #000000; border-left: 1px solid #000000; border-right: 1px solid #000000" align="center" valign=middle bgcolor="#337ab7"><b><font color="#000000">Environmental Safety Checklist</font></b></td>
