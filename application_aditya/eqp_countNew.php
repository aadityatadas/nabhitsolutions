	...
	<div class="col-lg-10" style="padding-left:0;">
		<table class="table table-bordered">
			...
			<tr>
				...
				<?php
$qry2 = mysqli_query($connect,"SELECT eqpid FROM tbl_eqp_mast")or die(mysqli_error($connect));
...
?>
		...
		<tr>
				<td style="text-align:left;" width="50%">Total No.of Equipment Under
Warranty</td>
<?php
					$sq2 = mysqli_query($connect,"SELECT COUNT(*) FROM tbl_eqp_mast WHERE (month(eqp_wty1) = '$month' OR month(eqp_wty1) < '$month' AND year(eqp_wty1) = '$yr' OR year(eqp_wty1) < '$yr') AND (month(eqp_wty2) > '$month' AND year(eqp_wty2) = '$yr' OR year(eqp_wty2) > '$yr')")or die(mysqli_error($connect));
...
Warranty</td>
	...
	$qry3_2 = mysqli_query($connect,"SELECT eqp_amcs FROM tbl_eqp_indic WHERE eqp_amcs = 'YES'")or die(mysqli_error($connect));
...
?>
				...
				$sql5 = mysqli_query($connect,"SELECT eqp_name FROM tbl_eqp_mast LEFT JOIN tbl_eqp_indic ON tbl_eqp_mast.eqpid = tbl_eqp_indic.eqpid WHERE (month(tbl_eqp_mast.eqp_amc1) = '$month' OR month(tbl_eqp_mast.eqp_amc1) < '$month' AND year(tbl_eqp_mast.eqp_amc1) = '$yr' OR year(tbl_eqp_mast.eqp_amc1) < '$yr') AND (month(tbl_eqp_mast.eqp_amc2) > '$month' AND year(tbl_eqp_mast.eqp_amc2) = '$yr' OR year(tbl_eqp_mast.eqp_amc2) > '$yr')")or die(mysqli_error($connect));
			...
			<tr>
				...
				<?php
						$month = date('m');
					$sqlBreak = mysqli_query($connect,"SELECT eqp_id FROM tbl_eqp_indic WHERE MONTH(eqp_brkdwn_date) = '$month'")or die(mysqli_error($connect));
			...
			<tr>
				...
				<?php
					...
					$qry2 = mysqli_query($connect,"SELECT * FROM tbl_eqp_indic WHERE (eqp_brkdwn_date BETWEEN '$frdt' AND '$todt')")or die(mysqli_error($connect));
