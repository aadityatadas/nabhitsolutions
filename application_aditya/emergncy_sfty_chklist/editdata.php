<?php
   ...
   $id=$_GET['id'];
	$qry = "SELECT * FROM `tbl_emergncy_sfty_chklst` WHERE `emergncy_sfty_chklst_id`='$id'";
		$res = mysqli_query($connect, $qry);
		$mem=mysqli_fetch_array($res);
		$count = $res->num_rows;
	

        $row[0]= 'emergncy_codes_numbers';
		$row[1]= 'roles_emrgy_cards';
		$row[2]= 'emergncy_code_alarm';
		$row[3]= 'mock_drills_check';
		$row[4]= 'emergncy_equipment_chklst';
...
<table cellspacing="0" border="0" class="table-show">
	...
	<tr>
		...
		<td style="border-top: 1px solid #000000; border-bottom: 1px solid #000000; border-left: 1px solid #000000; border-right: 1px solid #000000" align="center" valign=middle bgcolor="#337ab7"><b><font color="#000000">Emergency Prepareness checklist</font></b></td>
