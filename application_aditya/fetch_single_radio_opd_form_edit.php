...
if(isset($_POST["radio_opd_extra_id"]))
{
   
    if($_POST["action"]=="view"){
	 $query = "SELECT * FROM tbl_radio_opd_extra WHERE radio_opd_extra_id = '".$_POST["radio_opd_extra_id"]."'";  
      ...
      $row = mysqli_fetch_array($result);  
       $output = array();
		...
		$output["radio_opd_extra_id"] = $row["radio_opd_extra_id"];
    ...
    if(isset($_POST["user_idEdit"])){
      if($_POST["actionEdit"]=="update"){
      	...
      	$radio_opd_extra_id = $_POST["radio_opd_extra_id"];         
    ...
    $operation = $_POST['operation'];
		...
		$statement = $connection->prepare(
			...
			WHERE radio_opd_extra_id = :user_id "
		   );
		

		$result = $statement->execute(
			array(
				':user_id'		=>	$radio_opd_extra_id
			)
		);
