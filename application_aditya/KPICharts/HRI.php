...
<?php
                          ...
                          $qry = "SELECT COUNT(*) as total FROM  tbl_hr_indic WHERE year(hr_date) = '$yr'";
                            ...
                            $qrycomp = "SELECT COUNT(*) as comp FROM tbl_hr_indic LEFT JOIN tbl_hr_mast ON tbl_hr_mast.hrid = tbl_hr_indic.hrid WHERE hr_ctstat!=' 'AND year(hr_date) = '$yr' ";
                                $rescomp = mysqli_query($connect, $qrycomp);
                                $rowcomp=mysqli_fetch_assoc($rescomp);
                                // echo $rowdis['discharge'];
                                                      

                            $qrynotcomp = "SELECT COUNT(*) as notcomp FROM tbl_hr_indic LEFT JOIN tbl_hr_mast ON tbl_hr_mast.hrid = tbl_hr_indic.hrid WHERE hr_ctstat=' ' AND year(hr_date) = '$yr'";
...
<div class="col-lg-12">
  ...
  <script>
    ...
    var myConfig = {
      type: "pie",
      title: {
        text: 'Performance Of HR Indicator'
      },
      plot: {
        valueBox: {
          placement: 'out',
