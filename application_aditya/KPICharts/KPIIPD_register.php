      ...
      display: none;
                    ...
                    <?php
                          ...
                          $qry = "SELECT COUNT(*) as total  FROM tbl_huf WHERE year(huf_dadm) = '$yr'";
                            ...
                            $qrycomp = "SELECT COUNT(*) as comp FROM tbl_huf WHERE (huf_ddd='Discharge' AND huf_ddd!=' ') AND year(huf_dadm) = '$yr' ";
                                $rescomp = mysqli_query($connect, $qrycomp);
                                $rowcomp=mysqli_fetch_assoc($rescomp);
                              //  echo $rowcomp['comp'];
                                                      

                            $qrynotcomp= "SELECT COUNT(*) as notcomp FROM tbl_huf WHERE (huf_ddd='Death' OR huf_ddd='DAMA' OR huf_ddd=' ') AND year(huf_dadm) = '$yr' ";
...
<div class="col-lg-12">
  ...
  <script>
    ...
    var myConfig = {
      type: "pie",
      title: {
        text: 'Performance Of IPD Register'
      },
      plot: {
        valueBox: {
          placement: 'out',
