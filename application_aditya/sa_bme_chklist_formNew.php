...
<?php  foreach ($data as $key => $value) {  ?> 
	                            <div class="col-lg-12">
											...
											<div class="col-lg-8">		
												<input type="text" autocomplete="off" name="<?=$value?>" id="<?=$value?>" placeholder="<?=$value?>" class="form-control" />
											</div>
										</div>
...
<script>	
	$(document).ready(function() {
		...
		var dataTable = $('#dataTables-example').DataTable({
			...
			"order":[],
			"ajax":{
				url:"ward_round_cklist_d/fetch_icuchlst_audit.php",
				type:"POST"
			},
			"columnDefs":[
				{
	...
	// $("#audit_form2").submit(function(event){
			...
			if(count > 0)
			{
				if(confirm("Are you sure you want to Submit this?"))
				{
					...
					$.ajax({
						url:"ward_round_cklist_d/insert_patinet.php",
						method:'POST',
						data:new FormData(this),
						contentType:false,
						processData:false,
		...
		$(document).on('click', '.update', function(){
			...
			//$('#md2').show('fast');
			$.ajax({
				url:"ward_round_cklist_d/fetch_patient_icuchlst_audit.php",
				method:"POST",
		...
		$(document).on('submit', '#audit_form1', function(event){
				...
				if(s_size != '' && s_size!=0)
			{
				if(confirm("Are you sure you want to Submit this?"))
				{
					...
					$.ajax({
						url:"ward_round_cklist_d/search_patinet.php",
						method:'POST',
						data:new FormData(this),
						contentType:false,
						processData:false,
