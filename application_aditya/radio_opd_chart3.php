...
if(isset($_POST["frdate"],$_POST["todate"]))
{
	$data[] = array('Date','Average TAT turn around time');
		...
		// main table
	$sq = "SELECT DISTINCT invst_stat_date_time FROM tbl_radio_opd WHERE (invst_stat_date_time BETWEEN '".$_POST["frdate"]."' AND '".$_POST["todate"]."') ORDER BY invst_stat_date_time ASC";
	...
	// add on table
	$sq = "SELECT DISTINCT invst_stat_date_time FROM tbl_radio_opd_extra WHERE (invst_stat_date_time BETWEEN '".$_POST["frdate"]."' AND '".$_POST["todate"]."') ORDER BY invst_stat_date_time ASC";
	$rs = mysqli_query($connect, $sq);
	$date2 = array();
	while($res = mysqli_fetch_array($rs))
	{
	...
	{	
		...
		$tablename2 = 'tbl_radio_opd_extra';
