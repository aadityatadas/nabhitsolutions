...
if(isset($_POST["user_id"]))
{
	...
	$statement = $connection->prepare(
		"SELECT  tbl_huf.huf_id,tbl_huf.huf_pname,tbl_huf.huf_uhid,tbl_huf.huf_ipd , tbl_huf.huf_dadm , tbl_radio_ipd.* FROM tbl_huf
		LEFT JOIN tbl_radio_ipd ON tbl_huf.huf_id = tbl_radio_ipd.tbl_huf_id
...
// SELECT *
//  FROM tbl_huf
//  LEFT JOIN tbl_radio_ipd ON tbl_huf.huf_id = tbl_radio_ipd.tbl_uhf_id
//  WHERE `huf_id`= 2030
...
// LIMIT 1
	...
	foreach($result as $row)
	{
		...
		// $output["surg"] = $row["vent_spc"];
         ...
         $output["radio_ipd_id"] = $row["radio_ipd_id"];
		...
		$output["redos_rsn"] = $row["redos_rsn"];
		 ...
		 $output["clinical_correlation"] = $row["clinical_correlation"];
		  ...
		  //for add on
		$statement = $connection->prepare(
		"SELECT * FROM tbl_radio_ipd_extra
		WHERE tbl_radio_ipd_id = '".$row["radio_ipd_id"]."' 
		"
	);
	$statement->execute();
	$result = $statement->fetchAll();
    ...
    $count = 0;
     ...
     foreach ($result as $as => $row) {
		...
		$output["radio_ipd_extra_id"][$as] = $row["radio_ipd_extra_id"];
		$output["tbl_radio_lpd_id"][$as] = $row["tbl_radio_lpd_id"];
