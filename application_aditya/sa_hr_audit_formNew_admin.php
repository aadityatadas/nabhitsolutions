    ...
    <div id="wrapper">
        ...
        <td>
        	<label class="radio-inline">
					<input type="radio" name="<?=$value[1]?>" id="<?=$value[1]?>" value="P" checked>P
			</label>
			&nbsp;&nbsp;
			<label class="radio-inline">
					<input type="radio" name="<?=$value[1]?>" id="<?=$value[1]?>" value="N" >N
			</label>
			&nbsp;&nbsp;
			<label class="radio-inline">
	...
	$(document).ready(function() {
		...
		var tbl_hr_audit_date_id = $('#tbl_hr_audit_date_id').val();
		 ...
		 $(document).on('click', '.updateAudit', function(){
			...
			$('.main').hide('fast');
			$('.mainAudit').show('fast');
		// 	$('#dataTables-exampleAudit').DataTable({
		// 	"processing":true,
		// 	"serverSide":true,
		// 	"order":[],
		// 	"ajax":{
        ...
        "order":[],
			"ajax":{
				url:"hr_audit_d/fetch_all_data.php",
				type:"POST",
				data:{tbl:'tbl_hr_audit',user_id:user_id}
			},
			"columnDefs":[
				{
					"targets":[0, 3, 4],
