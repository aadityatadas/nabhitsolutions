                           ...
                           <table class="custom-table"  cellspacing="10" cellpadding="10" border="1" width="650px" align="center" style="border-color: #9DA2E2; text-align: center;" >
                                    ...
                                    <tr style="background-color: white;">
                                      <?php
                                            ...
                                            $tot= "SELECT COUNT(*) as total FROM tbl_medical_record_audit WHERE year(monthyear) = '$yr'";
                                              ...
                                              $qrycomp = "SELECT COUNT(*) as comp FROM tbl_medical_record_audit WHERE mr_without_dis_summary='Yes' AND year(monthyear) = '$yr' ";
                                                  $rescomp = mysqli_query($connect, $qrycomp);
                                                  $rowcomp=mysqli_fetch_assoc($rescomp);
                                                  // echo $rowcomp['comp'];
                                                                        

                                              $qrynotcomp = "SELECT COUNT(*) as notcomp FROM tbl_medical_record_audit WHERE (mr_without_dis_summary='No'OR mr_without_dis_summary='Na')   AND year(monthyear) = '$yr' ";
        ...
        <div class="form-group">
          <div class="col-lg-12">
            <div class="col-lg-8" style="padding-left:0;">
              ...
              <div class="col-lg-4">
                <button type="button" style="color: white;" name="search" id="search" class="btn btn-primary btn-sm" onclick="line_chart()" >SEARCH</button>
              </div>
            </div>
          </div>
        </div>
...
<script>  
  ...
  $(document).ready(function() {
    ...
    var dataTable = $('#dataTables-example').DataTable({
      ...
      "order":[],
      "ajax":{
        url:"medical_record_audit_d/fetch_all_month_details.php",
        type:"POST",
        data:{tbl:'tbl_medical_record_audit'}
      },
      "columnDefs":[
        {
          "targets":[0, 3, 4],
  ...
  // $("#audit_form2").submit(function(event){
    ...
    $(document).on('click', '.delete', function(){
      ...
      {
        $.ajax({
          url:"medical_record_audit_d/delete_single.php",
          method:"POST",
          data:{user_id:user_id,tbl:'tbl_medical_record_audit'},
          success:function(data)
          {
            alert(data);
            dataTable2.ajax.reload();
    ...
    $(document).on('click', '.deleteAll', function(){
      ...
      {
        $.ajax({
          url:"medical_record_audit_d/delete_all_monthyear.php",
          method:"POST",
          data:{user_id:user_id ,tbl:'tbl_medical_record_audit'},
          success:function(data)
          {
