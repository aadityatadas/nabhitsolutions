<?php
	...
	$frdt = date('Y-m-01');
	$todt = date('Y-m-31');
	$qry = mysqli_query($connect,"SELECT * FROM tbl_lab_ipd WHERE sample_rec_date BETWEEN '$frdt' AND '$todt'")or die(mysqli_error($connect));
	...
	//$i_count = $res["std"];
	$qry2 = mysqli_query($connect,"SELECT * FROM tbl_lab_ipd WHERE (sample_rec_date BETWEEN '$frdt' AND '$todt') AND cri_res_if_any ='Yes'")or die(mysqli_error($connect));
	...
	$qry3 = mysqli_query($connect,"SELECT * FROM tbl_lab_ipd WHERE (sample_rec_date BETWEEN '$frdt' AND '$todt') AND redos = 'Yes'")or die(mysqli_error($connect));
	...
	$qry4 = mysqli_query($connect,"SELECT * FROM tbl_lab_ipd WHERE (sample_rec_date BETWEEN '$frdt' AND '$todt') AND resp_err = 'Yes'")or die(mysqli_error($connect));
	$error = mysqli_num_rows($qry4);
	$pererror = ($error / $c_count) * 100 ;

	$pererror = is_nan($pererror)? 0 : $pererror;

	$qry5 = mysqli_query($connect,"SELECT * FROM tbl_lab_ipd WHERE (sample_rec_date BETWEEN '$frdt' AND '$todt') AND clinical_correlation = 'Yes'")or die(mysqli_error($connect));
