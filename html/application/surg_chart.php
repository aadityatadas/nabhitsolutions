			...
			<script type="text/javascript">  
			   ...
			   {  
					var data = google.visualization.arrayToDataTable([  
							  ['Status', 'Number'],  
							  <?php  
							  $query3 = "SELECT surg_symp_det, count(*) as d_numb FROM tbl_huf WHERE surg_dtos BETWEEN '$m-01' AND '$m-31' GROUP BY surg_symp_det";  
							  $result3 = mysqli_query($connect, $query3);
							  	  
							  while($row3 = mysqli_fetch_array($result3))  
							  {  
								   echo "['".$row3["surg_symp_det"]."', ".$row3["d_numb"]."],";  
							  }
							  ?>  
						 ]);  
					var options = {  
				...
				<table id="datatable">
					...
					<tbody>
						<?php 
							//index.php
							$mon = date('Y-m');
							$query = "SELECT DISTINCT date(surg_dtos) as dat FROM tbl_huf WHERE date(surg_dtos) BETWEEN '$mon-01' AND '$mon-31'";
							$result = mysqli_query($connect, $query);
							while($row = mysqli_fetch_array($result))
							{
								$query1 = "SELECT huf_id, count(*) as vid FROM tbl_huf WHERE date(surg_dtos) = '".$row["dat"]."'";
								$result1 = mysqli_query($connect, $query1);
								$row1 = mysqli_fetch_array($result1);
								
								$query2 = "SELECT surg_sp_ssi, COUNT(*) as spc FROM tbl_huf WHERE surg_sp_ssi = 'Yes' AND date(surg_dtos) = '".$row["dat"]."'";
