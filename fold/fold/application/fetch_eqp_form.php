...
$query = '';
$output = array();
$query .= "SELECT * FROM tbl_eqp_indic LEFT JOIN tbl_eqp_mast ON tbl_eqp_mast.eqp_id = tbl_eqp_indic.eqpid ";
...
foreach($result as $row)
{
	...
	$sub_array[] = $row["eqp_dttmbr"];
	// Fourth Attachment
	$qry = mysqli_query($connect,"SELECT * FROM tbl_eqp_upld WHERE eqp_id = '$centid' AND eqp_srno = '5'")or die(mysqli_error($connect));
	$rs = mysqli_num_rows($qry);
	if($rs > 0)
	{
		$sub_array[] = '<a title="Click here to view reports" style="cursor:pointer;text-decoration:none;" data-href="view_eqp_rpt5.php?rd='.$row["eqp_id"].'" data-target="_blank" class="edit_rpt"><b style="color:green;">'.$row["eqp_dttmrp"].'</b><br /><button class="btn btn-info btn-xs">View Report</button></a>';
	}else{
		$sub_array[] = '<a title="Click here to upload reports" style="cursor:pointer;text-decoration:none;" data-href="upd_eqp_rpt5.php?rd='.$row["eqp_id"].'" data-target="_blank" class="edit_rpt2"><b style="color:blue;">'.$row["eqp_dttmrp"].'</b><br /><button class="btn btn-warning btn-xs">Upload Report</button></a>';
	}
	//
