...
if(isset($_POST["user_id"]))
{
	...
	$statement = $connection->prepare(
		"SELECT  tbl_opdwttm.opdwttm_id,tbl_opdwttm.opdwttm_pname,tbl_opdwttm.opdwttm_uhid,tbl_opdwttm.opdwttm_opd , tbl_opdwttm.opdwttm_dttmr , tbl_lab_opd.* FROM tbl_opdwttm
		LEFT JOIN tbl_lab_opd ON
		 tbl_opdwttm.opdwttm_id = tbl_lab_opd.tbl_opdwttm_id
...
// SELECT *
//  FROM tbl_opdwttm
//  LEFT JOIN tbl_lab_ipd ON tbl_opdwttm.opdwttm_id = tbl_lab_ipd.tbl_opdwttm_id
...
// LIMIT 1
	...
	foreach($result as $row)
	{
		...
		// $output["surg"] = $row["vent_spc"];
         ...
         $output["lab_ipd_id"] = $row["lab_opd_id"];

		 $output["tbl_uhf_id"] = $row["tbl_opdwttm_id"];
		 ...
		 $output["clinical_correlation"] = $row["clinical_correlation"];
		   ...
		   $output["remarks"] = $row["remarks"];
				   ...
				   //for add on
		$statement = $connection->prepare(
		"SELECT * FROM tbl_lab_opd_extra
		WHERE tbl_lab_opd_id = '".$row["lab_opd_id"]."' 
		"
	);
	$statement->execute();
	$result = $statement->fetchAll();
    ...
    if(count($result)>0){
     ...
     foreach ($result as $as => $row) {
		...
		$output["lab_opd_extra_id"][$as] = $row["lab_opd_extra_id"];
		$output["tbl_lab_opd_id"][$as] = $row["tbl_lab_opd_id"];
