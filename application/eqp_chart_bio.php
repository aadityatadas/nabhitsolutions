...
if(isset($_POST["frdate"],$_POST["todate"]))
{
	...
	$sql2 = mysqli_query($connect,"SELECT eqp_name FROM tbl_eqp_mast_bio WHERE (month(eqp_wty1) = '$month' OR month(eqp_wty1) < '$month' AND year(eqp_wty1) = '$yr' OR year(eqp_wty1) < '$yr') AND (month(eqp_wty2) > '$month' AND year(eqp_wty2) = '$yr' OR year(eqp_wty2) > '$yr')")or die(mysqli_error($connect));
	$rs2 = mysqli_num_rows($sql2);	

	$sql3 = mysqli_query($connect,"SELECT * FROM tbl_eqp_indic_bio LEFT JOIN tbl_eqp_mast_bio ON tbl_eqp_mast_bio.eqpid = tbl_eqp_indic_bio.eqpid WHERE (tbl_eqp_indic_bio.eqp_csch1 = '".$_POST["frdate"]."' OR tbl_eqp_indic_bio.eqp_csch1 >= '".$_POST["todate"]."') AND (tbl_eqp_indic_bio.eqp_csch2 <= '".$_POST["frdate"]."' OR tbl_eqp_indic_bio.eqp_csch2 > '".$_POST["todate"]."')")or die(mysqli_error($connect));
