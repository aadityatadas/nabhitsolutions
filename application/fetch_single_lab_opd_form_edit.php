...
if(isset($_POST["lab_opd_extra_id"]))
{
   
    if($_POST["action"]=="view"){
	 $query = "SELECT * FROM tbl_lab_opd_extra WHERE lab_opd_extra_id = '".$_POST["lab_opd_extra_id"]."'";  
      ...
      $row = mysqli_fetch_array($result);  
       ...
       $output["prov_finl_daigEdit"] = $row["prov_finl_daig"];
		...
		$output["lab_opd_extra_id"] = $row["lab_opd_extra_id"];
		$output["tbl_lab_opd_id"] = $row["tbl_lab_opd_id"];
    ...
    if(isset($_POST["user_idEdit"])){
      if($_POST["actionEdit"]=="update"){
      	...
      	$lab_opd_extra_id = $_POST["lab_opd_extra_id"];         
    ...
    $operation = $_POST['operation'];
		...
		$statement = $connection->prepare(
			...
			WHERE lab_opd_extra_id = :user_id  "
		   );
		

		$result = $statement->execute(
			array(
				':user_id'		=>	$lab_opd_extra_id
			)
		);
