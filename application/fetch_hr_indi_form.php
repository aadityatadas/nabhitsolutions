...
if(isset($_POST["search"]["value"]))
{
	...
	$query .= 'OR tbl_hr_indic.hr_month LIKE "%'.$_POST["search"]["value"].'%" ';
...
foreach($result as $row)
{
	...
	if($row["hr_immune"] == 'Yes'){
		...
		if($rs > 0)
		{
			$sub_array[] = '<a title="Click here to view reports" style="cursor:pointer;text-decoration:none;" data-href="view_hr_rpt5.php?rd='.$row["hr_id"].'" data-target="_blank" class="edit_rpt"><b style="color:green;">'.$row["hr_immune"].'</b><br /><button class="btn btn-info btn-xs">View Report</button></a>';
		}else{
			$sub_array[] = '<a title="Click here to upload reports" style="cursor:pointer;text-decoration:none;" data-href="upd_hr_rpt5.php?rd='.$row["hr_id"].'" data-target="_blank" class="edit_rpt2"><b style="color:blue;">'.$row["hr_immune"].'</b><br /><button class="btn btn-warning btn-xs">Upload Report</button></a>';
		}
	}else{
